  ...
  <div class="min-h-screen bg-gradient-to-br from-gray-900 to-gray-800 py-12 px-4 sm:px-6 lg:px-8">
    <div class="max-w-3xl mx-auto">
      ...
      <div class="text-center mb-12">
        <h1 class="text-4xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-blue-500 mb-4">
          AI Code Generator
        </h1>
      ...
      <div class="bg-gray-800 rounded-xl p-6 shadow-lg mb-8 border border-gray-700">
        <div class="flex items-center justify-between mb-4">
          <label for="prompt" class="block text-sm font-medium text-gray-300">Your Code Request</label>
          <span class="text-xs text-gray-500">Free Tier (30k tokens/month)</span>
        </div>
        <textarea
          id="prompt"
          v-model="prompt"
        ...
        <div class="flex justify-between mt-4">
          ...
          <button
            @click="generateCode"
            :disabled="loading"
            class="flex items-center justify-center px-6 py-3 bg-gradient-to-r from-purple-600 to-blue-500 hover:from-purple-700 hover:to-blue-600 rounded-lg text-white font-medium shadow-lg hover:shadow-purple-500/20 transition-all disabled:opacity-50"
          >
      ...
      <!-- Output Area -->
      <div v-if="code" class="bg-gray-800 rounded-xl p-6 shadow-lg border border-gray-700 transition-all duration-300 relative">
        <div class="flex items-center justify-between mb-4">
          <div class="flex items-center space-x-2">
            <h2 class="text-lg font-medium text-gray-300">Generated Code</h2>
            <span v-if="language" class="bg-blue-500 text-white text-xs px-2 py-1 rounded-full">
              {{ language }}
            </span>
          </div>
          <div class="flex space-x-2">
        ...
        <div class="relative">
          <pre class="p-4 overflow-x-auto bg-gray-900 rounded-lg text-gray-200 font-mono text-sm border border-gray-700"><code>{{ typedCode || " " }}</code></pre>
          <div v-if="copied" class="absolute top-2 right-2 bg-gray-800 text-xs text-green-400 px-2 py-1 rounded-md">
            Copied!
          </div>
        </div>
      </div>
      ...
      <!-- Empty State -->
      <div v-else class="bg-gray-800/50 rounded-xl p-12 text-center border-2 border-dashed border-gray-700">
        ...
        <h3 class="mt-2 text-sm font-medium text-gray-400">Your AI-generated code will appear here</h3>
      </div>
    </div>
  </div>
</template>
